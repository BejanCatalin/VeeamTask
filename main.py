...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("source", metavar="SRC", help="Path to source directory")
    parser.add_argument("destination", metavar="DST", help="Path to destination directory")
...
def copy_files(source_folder: str, destination_folder: str, files: List[str]) -> None:
    ...
    for file_name in files:
        ...
        if os.path.isfile(source_path):
            ...
            os.remove(source_path)  # delete the transferred file from the source folder
...
def main() -> None:
    ...
    logging.info(f"Folder synchronization tool started at {now} with loop time {args.loop_time} minutes")
    ...
    while True:
        ...
        logging.info(f"Waiting for {time_to_sleep} seconds until the next synchronization...")
